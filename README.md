...
| [`apps/python/vendor-discovery-agent`](apps/python/vendor-discovery-agent/) | Python | Zero-input vendor sourcing: discovers local businesses from OpenStreetMap by product and city with no lead list to prepare, then runs a consent-gated two-round CALL-E qualification and follow-up campaign with a dry-run default, business-hours gating, and cancellable scheduled retries. |
